...
class GlobalDatabase():
    def __init__(self):
        self.data = sqlite3.connect('db/bot.db', check_same_thread=False)
    ...
    def get_quest_list(self, point):
        x, y = point
        data = self.base.execute(GET_QUEST_FROM_POSITION.format(x, y)).fetchall()
        if data==[]:
